{% macro source_profiling(source_name, table_name,check_na =True, date_incremental=None, condition= None) %}
  {{ return(adapter.dispatch('source_profiling')(source_name, table_name, check_na, date_incremental, condition)) }}
{% endmacro %}

{% macro bigquery__source_profiling(source_name, table_name,check_na, date_incremental, condition) %}
...
{% for column in columns %} 
     select distinct
       ...
       '{{column.dtype}}' as data_type,
        ...
        COUNT( {{ column.name }}) OVER(PARTITION BY partition_field ) as count, 
        COUNT(DISTINCT {{ column.name }}) OVER(PARTITION BY partition_field ) as count_distinct,
        COUNTIF({{ column.name }} IS NULL) OVER(PARTITION BY partition_field) as count_null,
        IEEE_DIVIDE(COUNTIF({{ column.name }} IS NULL) OVER(PARTITION BY partition_field ), COUNT( {{ column.name }} ) OVER(PARTITION BY partition_field )) as null_ratio, 
        IEEE_DIVIDE(COUNT(DISTINCT {{ column.name }} ) OVER(PARTITION BY partition_field ), COUNT( {{ column.name }}) OVER(PARTITION BY partition_field )) as distinct_ratio, 
        {%if check_na %}
           {%- if column.dtype in ('STRING') -%}
        COUNTIF( lower({{ column.name }}) in ('nan','n/a','-infinty','na','','-','__','--','_')) OVER(PARTITION BY partition_field) as count_na, 
           {%- else -%}
        null as count_na,
           {%- endif -%}
        {% endif %}
        {% if column.dtype in ('INT64','FLOAT64','NUMERIC','BIGNUMERIC') %}
        AVG({{ column.name }}) OVER(PARTITION BY partition_field ) as average,
        SUM({{ column.name }}) OVER(PARTITION BY partition_field ) as sum,
        STDDEV(COALESCE ({{ column.name }}, 0 )) OVER(PARTITION BY partition_field ) as  standard_dev,
        MIN(COALESCE ({{ column.name }}, 0 )) OVER (PARTITION BY partition_field)  as min,
...
{% macro snowflake__source_profiling(source_name, table_name, date_incremental, check_na ,condition) %}
{% set source_relation = source(source_name, table_name) %}
{% set columns = adapter.get_columns_in_relation(source_relation) %}
{% if execute %}    
    {% set create_table_query %}
